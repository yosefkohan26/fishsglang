...
def tts_engine_next(request_id: str, output: Any) -> str | None:
    """Route to vocoder for non-streaming; terminate for streaming.

    Streaming requests already delivered audio via stream chunks,
    so the full vocoder re-decode is redundant.
    """
    del request_id
